...
# ─────────────────────────────────────────────
# НОВЫЙ МОДУЛЬ: CryptoBot Payment (только добавлено)
# ─────────────────────────────────────────────
def create_crypto_invoice(user_id: int, plan: str):
    """Создание invoice через CryptoBot"""
    prices = {
...
# ─────────────────────────────────────────────
# НОВЫЕ CALLBACKS ДЛЯ CRYPTO (добавлено)
# ─────────────────────────────────────────────
...
def cb_process_crypto(call):
    ...
    url = invoice.get("bot_invoice_url") or invoice.get("pay_url")
    m = InlineKeyboardMarkup()
    m.add(InlineKeyboardButton("💳 Open Payment in Telegram", url=url))
